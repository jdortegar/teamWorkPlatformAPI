...
import * as teamMembersTable from './teamMembersTable';
...
export const getTeamsWithTeamMembers = async (req, orgId) => {
    const teams = await getTeamsBySubscriberOrgId(req, orgId);
    for (let i = 0; i < teams.length; i++) {
        const teamMembers = await teamMembersTable.getTeamMembersByTeamId(req, teams[i].teamId);
        teams[i].teamMembers = teamMembers;
    }
    return teams;
}
